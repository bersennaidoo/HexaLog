...
func setupTest(t *testing.T, fn func(*app.Config)) (
	client api.LogClient,
	cfg *app.Config,
	teardown func(),
) {
	...
	require.NoError(t, err)
	clog, err := app.NewLog(dir, config.Config{})
	require.NoError(t, err)
	cfg = &app.Config{
		CommitLog: clog,
	}
...
func testProduceConsume(t *testing.T, client api.LogClient, config *app.Config) {
